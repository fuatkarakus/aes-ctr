...
public class Utils {
    ...
    public static void splitFile() throws IOException {
        ...
        try (FileInputStream fis = new FileInputStream(f);
            ...
            while ((bytesAmount = bis.read(buffer)) > 0) {
                ...
                String filePartName = String.format("%s.part%03d", fileName, partCounter++);
    ...
    public static List<File> getParts() throws IOException {
        List<File> results = new ArrayList<>();

        File[] files = new File(Objects.requireNonNull(Utils.class.getClassLoader().getResource("./")).getPath()).listFiles();
